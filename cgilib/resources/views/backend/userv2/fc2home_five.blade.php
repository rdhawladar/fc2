    ...
    <link href="{{ URL::asset('/kachamal/css/green.css') }}" rel="stylesheet">
                  ...
                  <div class="x_content">
					...
					@if(isset($downloadLinks[0])) 
                     <article class="media event">
                      ...
                      <div class="media-body">
                        <a class="title" href="{{ url('/user/copy-url/'. $one['id'] .'/21') }}" target="_blank">{{ $one['title_url'] }}</a>
                        <p>{{ $one['url'] }}
                        </br>
                     {{ $downloadLinks[0] }}<br/>
                        	<form method="post" action="{{ url('/user/file-download') }}">
								...
								@endif
								   <a class="btn btn-xs btn-info" href="{{ url('/user/copy-url/'. $one['id'] .'/21') }}" target="_blank">Download Thumbnail</a>
					...
					@if(isset($downloadLinks[1])) 
                     <article class="media event">
                      ...
                      <div class="media-body">
                        <a class="title" href="{{ url('/user/copy-url/'. $two['id'] .'/22') }}" target="_blank">{{ $two['title_url'] }}</a>
                        <p>{{ $two['url'] }}
                        </br>
                     {{ $downloadLinks[1] }}<br/>
					<form method="post" action="{{ url('/user/file-download') }}">
						...
						<i class="fa fa-thumbs-o-up"></i>
						@endif
						<a class="btn btn-xs btn-info" href="{{ url('/user/copy-url/'. $two['id'] .'/22') }}" target="_blank">Download Thumbnail</a>                     
					</form>
                        </p>
                      </div>
                    </article> 
					...
					@if(isset($downloadLinks[2])) 
                     <article class="media event">
                      ...
                      <div class="media-body">
                        <a class="title" href="{{ url('/user/copy-url/'. $three['id'] .'/23') }}" target="_blank">{{ $three['title_url'] }}</a>
                        <p>{{ $three['url'] }}
                        </br>
							{{ $downloadLinks[2] }}<br/>
                        	<form method="post" action="{{ url('/user/file-download') }}">
								...
								<i class="fa fa-thumbs-o-up"></i>
								@endif
								 <a class="btn btn-xs btn-info" href="{{ url('/user/copy-url/'. $three['id'] .'/23') }}" target="_blank">Download Thumbnail</a>
					...
					@if(isset($downloadLinks[3])) 
                     <article class="media event">
                      ...
                      <div class="media-body">
                        <a class="title" href="{{ url('/user/copy-url/'. $four['id'] .'/24') }}" target="_blank">{{ $four['title_url'] }}</a>
                        <p>{{ $four['url'] }}
                        </br>
							{{ $downloadLinks[3] }}<br/>
                        	<form method="post" action="{{ url('/user/file-download') }}">
								...
								<i class="fa fa-thumbs-o-up"></i>
								@endif
								  <a class="btn btn-xs btn-info" href="{{ url('/user/copy-url/'. $four['id'] .'/24') }}" target="_blank">Download Thumbnail</a>
					...
					@if(isset($downloadLinks[4])) 
                     <article class="media event">
                      ...
                      <div class="media-body">
                        <a class="title" href="{{ url('/user/copy-url/'. $five['id'] .'/25') }}" target="_blank">{{ $five['title_url'] }}</a>
                        <p>{{ $five['url'] }}
                        </br>
							{{ $downloadLinks[4] }}<br/>
                        	<form method="post" action="{{ url('/user/file-download') }}">
								...
								<i class="fa fa-thumbs-o-up"></i>
								@endif
								   <a class="btn btn-xs btn-info" href="{{ url('/user/copy-url/'. $five['id'] .'/25') }}" target="_blank">Download Thumbnail</a>
        ...
        <style>
			...
			<p>
			        ...
			        <li><a class="btn btn-xs btn-danger" href="{{ url('/user/home/five') }}">5</a></li>
