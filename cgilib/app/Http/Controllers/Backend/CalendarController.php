...
{     
	...
	{	
		...
		return Datatables::of($events)
			...
			->editColumn('id', function($result){
			   return  '<a class="btn btn-success" href="' . url('/admin/calendar-event-edit/'.  date("Y-m-d",strtotime($result->created_at)) .'/'. $result->id ) .'">Edit</a>';
			})
			->make(true);
	}
    ...
    public function calendar_event_feedback_list_data_table()
    {
        ...
        $events = NewsPoll::leftJoin('event_table','event_table.id','=','news_poll.news_id')
                    ...
                    ->select(['news_poll.id', 'event_table.event_title','calendar_poll_request_form.name','calendar_poll_request_form.email','calendar_poll_request_form.message','news_poll.vote_for','news_poll.created_at'])
